...
GuiTransform::GuiTransform(): m_uid{GenerateUID(UIDType::GUI)}, m_global_position{0,0}, m_local_position{0,0}, m_dimensions{0,0},
...
GuiTransform::GuiTransform(const GuiTransform& other): m_uid{GenerateUID(UIDType::GUI)}, m_global_position{other.m_global_position}, m_local_position{other.m_local_position}, m_dimensions{other.m_dimensions},
...
void GuiTransform::BaseDimensions(const Dimensions& new_dimensions)
{
    ...
    if(m_parent != nullptr)
    {
        ...
        if(m_parent_viewport)
        {
            ...
            return {m_parent->GlobalPosition().x + m_local_position.x - m_parent_viewport->GlobalPosition().x,
                    m_parent->GlobalPosition().y + m_local_position.y - m_parent_viewport->GlobalPosition().y, m_dimensions.w, m_dimensions.h };
        }
        else
            return {m_parent->GlobalPosition().x + m_local_position.x, m_parent->GlobalPosition().y + m_local_position.y, m_dimensions.w, m_dimensions.h };
    }
    else
        return {m_global_position.x, m_global_position.y, m_dimensions.w, m_dimensions.h };
}
