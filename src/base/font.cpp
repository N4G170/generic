...
{
    m_font_ptr = TTF_OpenFont(m_source_path.data(), font_size);
...
{
    ResetTextureToTransparency(m_renderer_ptr, output_texture);
...
void Font::CreateFontGlyphSheet()
{
    ...
    int texture_width{0};
    int font_height{TTF_FontHeight(m_font_ptr)};
    //create glyphs surfaces
    for(int code : decoded_chars)
    {
        //normal
        TTF_SetFontStyle(m_font_ptr, TTF_STYLE_NORMAL);
        RenderGlyph(glyph_surfaces[code].normal, texture_width, code);

        //bold
        TTF_SetFontStyle(m_font_ptr, TTF_STYLE_BOLD);
        RenderGlyph(glyph_surfaces[code].bold, texture_width, code);

        //italic
        TTF_SetFontStyle(m_font_ptr, TTF_STYLE_ITALIC);
        RenderGlyph(glyph_surfaces[code].italic, texture_width, code);

        //italic bold
        TTF_SetFontStyle(m_font_ptr, TTF_STYLE_ITALIC | TTF_STYLE_BOLD);
        RenderGlyph(glyph_surfaces[code].italic_bold, texture_width, code);
    }
    TTF_SetFontStyle(m_font_ptr, TTF_STYLE_NORMAL);//reset
    ...
    //apply transparency
    ResetTextureToTransparency(m_renderer_ptr, m_font_texture);
    ...
    for(auto pair : glyph_surfaces)//TODO::join this loop with the one above
    {
        ...
        TTF_GlyphMetrics(m_font_ptr, pair.first, nullptr, nullptr, nullptr, nullptr, &adv);
...
{
    //i = unicode
    SDL_Surface* glyph_surface = TTF_RenderGlyph_Blended(m_font_ptr, char_unicode, {255,255,255,255});

    int ww{0};
    Uint16 text[] = {static_cast<Uint16>(char_unicode),0};
    TTF_SizeUNICODE(m_font_ptr, text, &ww, nullptr);
